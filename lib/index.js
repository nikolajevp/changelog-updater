...
function getRepositoryUrl(repository, version) {
	...
	if (typeof repository === 'object' && repository.url) {
		 ...
		 * - SSH: "git@github.com:repo/user-name.git"
		 */
		[, type, name] = repository.url.match(
			 ...
			 * Group 3: (?:\/|:) - slash or colon after scheme and domain, required, non-matching
			 * Group 4: ([\w-]+\/[\w-]+) - username/repository, e.g. `nikolajevp/changelog-updater`, required, matching
			 * Group 5: (?:\.git)? - suffix `.git`, optional, non-matching
			 */
			/^(?:[\w+]+?:\/\/)?(.+)(?:\/|:)([\w-]+\/[\w-]+)(?:\.git)?$/
		);
	}
...
function updateChangelog(options) {
	...
	if (options.check) {
		if (isChanged(changelog)) {
			...
			throw new Error(
				`No changes detected under [Unreleased] in ${changelogFilename}!\nPlease use ### headings. See: https://keepachangelog.com`
			);
		}
	}
